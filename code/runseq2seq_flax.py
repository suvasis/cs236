...
adapted from https://github.com/huggingface/transformers/blob/master/examples/flax/summarization/run_summarization_flax.py
Script adapted from run_summarization_flax.py

This script can be modified  from run_summarization_flax.py
"""
...
from project.text import TextNormalizer
...
class ModelArguments:
    ...
    # model weights should be initialized and trained
...
class DataTrainingArguments:
    ...
    #taking column "caption" image text
    ...
    #image encoding we got from VQGAN
    ...
    #play with rthis 128 is ok for now
...
def data_loader(
    ...
    print(" >>>>>>>>>>>> batch_size,  len(dataset) , steps_per_epoch >>>>", batch_size, len(dataset) , steps_per_epoch)
...
def main():
    ...
    data_files = {
        "encoder" : "/home/minasm/suvasis/tools/cs236_project/code/encoding/encodedfile/images-encoded_10m_886480.tsv",
        "train": data_args.train_file, #'**/train/train.tsv', #data_args.train_file,
        "validation": data_args.validation_file, #'**/validation/validation.tsv', #data_args.validation_file,
    }
    #dataset = load_dataset(
    #    #data_args.dataset_repo_or_path,
    #    path='/home/minasm/suvasis/tools/cs236_project/code/encoding/encodedfile/',
    #    #ignore_verifications=True,
    #    #data_files='/home/minasm/suvasis/tools/cs236_project/code/encoding/imagedata/',
    ...
    #print(" new model >>>>>>> MODEL >>>> ", model)

    #print(f"TPUs: {jax.device_count()}")
    #assert jax.device_count() == 8, "TPUs in use, please check running processes"
    assert jax.device_count() == 1, "TPUs in use, please check running processes"
    ...
    def shift_tokens_right(input_ids: np.array, decoder_start_token_id: int):
        ...
        #a1 = np.asarray(a)
        #print("TEST", a1.shape)
        #a2 = np.zeros(a1.shape,dtype='int')
        #print("TEST", a2)
        #exit(0)
        #print(" input_ids shape >>>>> ", input_ids.shape)
        shifted_input_ids = np.zeros(input_ids.shape, dtype="int")
        #print(" shifted_input_ids GGGGGGGGGGGGGGGGG 1",shifted_input_ids)
        #print(" shifted_input_ids GGGGGGGGGGGGGGGGG ",input_ids[0][:, :-1])
        #print(" shifted_input_ids GGGGGGGGGGGGGGGGG 2",input_ids[0])
        #print(" shifted_input_ids GGGGGGGGGGGGGGGGG 3",input_ids[:-1])
        #ori
        #shifted_input_ids[:, 1:] = input_ids[:, :-1]
        #shifted_input_ids[:, 0] = decoder_start_token_id

        #print(">>>>>>>>>>>>>>9", shifted_input_ids)
        shifted_input_ids[1:] = input_ids[:-1]
        shifted_input_ids[:1] = decoder_start_token_id
        #print(">>>>>>>>>>>>>>10", shifted_input_ids)
        #print(" shifted_input_ids GGGGGGGGG 4",shifted_input_ids)
    ...
    def preprocess_function(examples):
        ...
        #print(" preprocess_function inputs >>>>>> 1",inputs)
        ...
        #print(" model_inputs>>>>>>>>>>> START <<<<<<<<<<<<<<<<<<<< ")
        print(" model_inputs>>>>>>>>>>> 2", model_inputs)
        print(" model_inputs>>>>>>>>>>> 3", model_inputs["input_ids"].shape)
        ...
        #print(" encoding column >>>>>>>>>>> 3", encoding_column)
        #print(" examples >>>>>>>>>>> START <<<<<<<<<<<<<<<<<<<< ")
        #print(" examples >>>>>>>>>>> 5", examples)
        #print(" examples >>>>>>>>>>> END <<<<<<<<<<<<<<<<<<<< ")
        #print(" labels >>>>>>>>>>> START <<<<<<<<<<<<<<<<<<<< ")
        labels = examples[encoding_column]
        #print(" labels >>>>>>>>>> 6", len(labels))
        labels1 = labels[1:-1]
        arr = labels1.split(',')
        integer_map = map(int, arr) #Maps each string to an int.
        integer_list = list(integer_map) #Converts mapped output to a list of ints.
        #print(integer_list)
        #print(" labels >>>>>>>>>> 6", len(integer_list))
        #print(" labels >>>>>>>>>>> END <<<<<<<<<<<<<<<<<<<< ")
        labels = np.asarray(integer_list)
        #labels = np.array(labels1)
        #print(" labels as an array >>>>>>>>>> 3 ", labels.shape)
        #print(" labels as an array >>>>>>>>>> 4 ", labels)


        # We need the labels, in addition to the decoder_input_ids, for the compute_loss function
        model_inputs["labels"] = labels
        #print(" model_inputs>>>>>>>>>>> 2", len(model_inputs))

        # In our case, this prepends the bos token and removes the last one
        #                                                                       16384
        #print(" labels ", labels, " model.config.decoder_start_token_id >>>> ",model.config.decoder_start_token_id)
        decoder_input_ids = shift_tokens_right(
            labels, model.config.decoder_start_token_id
        )
        model_inputs["decoder_input_ids"] = decoder_input_ids
        print(" >>>>>>>>> final ",model_inputs["decoder_input_ids"])
        print(" >>>>>>>>> final1 ",len(model_inputs) , model_inputs["input_ids"].shape) #(1,128)

        #what the fuck is going on with this shape why it is (1,1,128) in the config?
    ...
    if training_args.do_train:
        ...
        #https://huggingface.co/transformers/_modules/transformers/models/bart/configuration_bart.html#BartConfig
        #
        #suvasis uncomment
    ...
    def train_step(state, batch):
        ...
        def compute_loss(params, batch):
            ...
            print(">>>>>>>>> labels 833 ",labels)
            print(">>>>>>>>> input_ids 8333 ",batch["input_ids"])
            logits = state.apply_fn(
                **batch, params=params, dropout_rng=dropout_rng, train=True
            )[0]
            print(">>>>>>>>> logits 834 ",logits)
            loss = loss_fn(logits, labels)
            return loss

        grad_fn = jax.value_and_grad(compute_loss)
        print(">>>>>>>>>>>>> state.params, batch : " , state.params, " : ", batch)
    ...
    for epoch in epochs:
        ...
        for batch in tqdm(
            ...
            print(" >>>>>>>>>>>> 111111 ",state, " >>>>> ",batch)
            #suvasis
            #state, train_metric = p_train_step(state, batch)
            state, train_metric = state, batch
            step = unreplicate(state.step)
        ...
        epochs.write(
            ...
            f"Epoch... ({epoch + 1}/{num_epochs} | Loss: , Learning Rate: )"
        )
